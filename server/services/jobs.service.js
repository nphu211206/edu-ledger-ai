 ...
 * @param {sql.Transaction | sql.Request} transactionOrRequest - Transaction (nếu đang chạy) hoặc pool.request().
...
const checkJobOwnership = async (jobId, recruiterId, transactionOrRequest) => {
    const functionName = 'jobsService.checkJobOwnership';
    try {
        // 1. Kiểm tra đối tượng truy vấn
        if (!transactionOrRequest || typeof transactionOrRequest.input !== 'function' || typeof transactionOrRequest.query !== 'function') {
            logError(functionName, new Error('Đối tượng truy vấn CSDL (transaction hoặc request) không hợp lệ.'), { jobId, recruiterId });
            throw new Error('Lỗi nội bộ: Đối tượng truy vấn CSDL không hợp lệ khi kiểm tra quyền.');
        }

        // 2. Tạo request mới từ context đã có (transaction hoặc pool)
        // Bằng cách này, nó có thể tham gia vào một transaction đang chạy
        const req = new sql.Request(transactionOrRequest);

        // 3. Query kiểm tra quyền
        const result = await req
            .input('jobId_checkOwner', sql.Int, jobId) // Dùng tên param duy nhất
            .input('recruiterId_checkOwner', sql.Int, recruiterId) // Dùng tên param duy nhất
            .query('SELECT 1 FROM Jobs WHERE id = @jobId_checkOwner AND recruiterId = @recruiterId_checkOwner');

        // 4. Trả về kết quả
        return result.recordset.length > 0;

    } catch (error) {
        logError(functionName, error, { jobId, recruiterId });
        // Ném lại lỗi để transaction bên ngoài có thể rollback
        throw new Error(`Lỗi khi kiểm tra quyền sở hữu tin tuyển dụng ID ${jobId}.`);
    }
};
