...
const { checkJobOwnership } = require('./jobs.service'); 

const { logError } = require('../utils/helpers.js');
const VALID_APPLICATION_STATUSES = ['Pending', 'Reviewed', 'Offered', 'Rejected', 'Hired', 'Withdrawn'];

/** Log lỗi */
const logDbError = (functionName, error, context = {}) => {
    // Gọi hàm logError từ helpers
    logError(`applications.service.${functionName}`, error, context);
};
/**
...
const updateApplicationStatus = async (applicationId, recruiterId, newStatus, notes = null) => {
    ...
    // ===>>> KIỂM TRA NÀY GIỜ SẼ CHẶN 'Interviewing' <<<===
    if (!newStatus || !VALID_APPLICATION_STATUSES.includes(newStatus)) {
        throw new Error(`Trạng thái mới không hợp lệ. Chỉ chấp nhận: ${VALID_APPLICATION_STATUSES.join(', ')}. (Để mời phỏng vấn, vui lòng dùng tính năng 'Mời PV').`);
    }
    ...
    try {
        ...
        // dùng checkJobOwnership từ jobs.service.js
        const isOwner = await checkJobOwnership(jobId, recruiterId, transaction); // Truyền transaction vào
        if (!isOwner) {
