// File: client/src/pages/recruiter/CreateJobPage.jsx
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { Helmet } from 'react-helmet-async';
import { createJob } from '../../services/api'; // Import hàm API

// --- COMPONENT CON ---
const FormInput = ({ id, label, ...props }) => (
    <div>
        <label htmlFor={id} className="block text-sm font-medium text-gray-300 mb-1">{label}</label>
        <input id={id} className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-blue-500" {...props} />
    </div>
);
...
const FormTextarea = ({ id, label, ...props }) => (
    <div>
        <label htmlFor={id} className="block text-sm font-medium text-gray-300 mb-1">{label}</label>
        <textarea id={id} rows="8" className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-blue-500" {...props}></textarea>
    </div>
);
...
const SkillInput = ({ skills, setSkills }) => {
    ...
    return (
        <div>
            <label className="block text-sm font-medium text-gray-300 mb-1">Kỹ năng yêu cầu (nhấn Enter để thêm)</label>
            <div className="flex flex-wrap items-center gap-2 p-2 bg-gray-700 border border-gray-600 rounded-md">
                {skills.map(skill => (
                    <div key={skill} className="flex items-center gap-2 bg-blue-900 text-blue-300 text-sm font-semibold px-3 py-1 rounded-full">
                        {skill}
                        <button type="button" onClick={() => removeSkill(skill)} className="text-blue-200 hover:text-white">&times;</button>
                    </div>
                ))}
                <input
                    type="text"
...
export default function CreateJobPage() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!formData.title || !formData.description || skills.length === 0) {
            setError('Vui lòng điền đầy đủ Tiêu đề, Mô tả và ít nhất một Kỹ năng.');
            return;
        }
    ...
    return (
        <>
            <Helmet>
                <title>Đăng tin tuyển dụng mới | EduLedger AI</title>
            </Helmet>
                    ...
                    <form onSubmit={handleSubmit} className="bg-gray-800 p-8 rounded-xl border border-gray-700 space-y-6">
                        ...
                        <div className="flex justify-end pt-4">
                            <button
                                type="submit"
                                disabled={isLoading}
                                className="w-full md:w-auto flex justify-center py-3 px-8 border border-transparent rounded-lg shadow-sm text-base font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:bg-gray-500"
                            >
