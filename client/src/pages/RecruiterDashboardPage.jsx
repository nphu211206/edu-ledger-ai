...
const Spinner = ({ size = 'h-8 w-8' }) => (
    <div className="flex justify-center items-center">
        <div className={`animate-spin rounded-full border-b-2 border-blue-400 ${size}`}></div>
    </div>
);
...
const EmptyState = ({ title, message }) => (
    <div className="text-center py-20 px-6 bg-gray-800 rounded-xl border-2 border-dashed border-gray-700">
...
export default function RecruiterDashboardPage() {
    ...
    // Lấy thông tin nhà tuyển dụng khi vào trang
    useEffect(() => {
        const fetchRecruiterData = async () => {
            ...
            try {
                ...
                localStorage.removeItem('token'); // Token hỏng, xóa đi
                navigate('/recruiter/login');
            } finally {
    ...
    const handleRemoveCriterion = (index) => {
        if (searchCriteria.length <= 1) return; // Luôn giữ lại ít nhất 1 dòng
        const values = [...searchCriteria];
    ...
    return (
        <div className="bg-gray-900 min-h-screen text-white p-4 sm:p-6 lg:p-8">
            <div className="max-w-7xl mx-auto">
                <h1 className="text-3xl font-bold text-white mb-2">Chào mừng, {recruiter?.name}!</h1>
                <p className="text-gray-400 mb-8">Đây là trung tâm điều khiển của bạn. Hãy bắt đầu tìm kiếm những tài năng sáng giá nhất.</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <aside className="lg:col-span-4 xl:col-span-3">
                        <div className="sticky top-24 bg-gray-800 p-6 rounded-xl border border-gray-700">
                            ...
                            <form onSubmit={handleSearch} className="space-y-4">
                                    ...
                                    <div className="flex items-end gap-2" key={index}>
                                        <div className="flex-grow">
                                            ...
                                            <input
                                                ...
                                                className="w-full mt-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                            />
                                        </div>
                                        <div>
                                            ...
                                            <input
                                                ...
                                                onChange={e => handleCriteriaChange(index, e)}
                                                min="0" max="100"
                                                className="w-20 mt-1 px-2 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                                            />
                                        </div>
                                ...
                                <button type="button" onClick={handleAddCriterion} className="w-full flex items-center justify-center gap-2 text-sm py-2 text-blue-400 hover:bg-gray-700 rounded-md transition">
                                    <PlusIcon /> Thêm tiêu chí
                                </button>
                                <button type="submit" className="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-700 transition disabled:bg-gray-600 flex justify-center" disabled={isSearching}>
                                    {isSearching ? <Spinner size="h-5 w-5" /> : 'Tìm kiếm ứng viên'}
                                </button>
                            </form>
                        </div>
                    </aside>
                    ...
                    <main className="lg:col-span-8 xl:col-span-9">
                        {isSearching ? (
                            ...
                            <EmptyState title="Bắt đầu tìm kiếm" message="Sử dụng bộ lọc bên trái để tìm các ứng viên có kỹ năng đã được AI xác thực." />
                        ) : searchResults.length === 0 ? (
                            <EmptyState title="Không tìm thấy kết quả" message="Hãy thử điều chỉnh hoặc giảm bớt các tiêu chí tìm kiếm của bạn." />
                        ) : (
